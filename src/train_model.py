...
class DataLoader:
    def __init__(self):  # array: data - time - features - has crimed - type_data
        osm_data = pd.read_csv("../data/osm_data_cell_onehot.csv")
        columns_to_drop = (osm_data.sum(axis=0) > 1000)
    ...
    def load_data(self):
        ...
        return train[train.columns[2:-1]],  train[train.columns[-1]], test[test.columns[2:-1]],  test[test.columns[-1]]
